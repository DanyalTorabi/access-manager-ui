...
**T6** — Application routing and layout (GitHub [#1](https://github.com/DanyalTorabi/access-manager-ui/issues/1))
...
- `src/App.tsx` — `RouterProvider` wrapping `QueryClientProvider`
- `src/pages/DomainDetailLayout.tsx` — layout wrapper that prefetches the domain record and renders sub-nav in the sidebar
- `src/components/Sidebar.tsx` — 56px fixed left sidebar; top-level "Domains" link; domain sub-nav (← All Domains, Users, Groups, Resources, Access Types, Permissions); dark-mode toggle in footer
...
5. Wrap app in `RouterProvider` + `QueryClientProvider` in `src/main.tsx`
...
- Edited: `src/App.tsx`, `src/main.tsx`
